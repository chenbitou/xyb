...
class AdminEnrollService extends BaseProjectAdminService {
	...
	async enrollJoinByAdmin({
		...
		this.AppError('[用车]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	...
	async getAdminEnrollList({
		...
		let fields = 'ENROLL_DAYS,ENROLL_TITLE,ENROLL_CATE_ID,ENROLL_CATE_NAME,ENROLL_EDIT_TIME,ENROLL_ADD_TIME,ENROLL_ORDER,ENROLL_STATUS,ENROLL_VOUCH,ENROLL_JOIN_CNT,ENROLL_EDIT_SET,ENROLL_CANCEL_SET,ENROLL_QR,ENROLL_OBJ';
	...
	/** 取消登记 */
	async delEnrollJoin(admin, enrollJoinId) {
		this.AppError('[用车]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	...
	async statDayCnt(enrollId) {
		...
		let where = {
			DAY_ENROLL_ID: enrollId,
			day: ['>=', timeUtil.time('Y-M-D')]
		}
		let cnt = await DayModel.count(where);
		await EnrollModel.edit(enrollId, { ENROLL_DAY_CNT: cnt });
	}
